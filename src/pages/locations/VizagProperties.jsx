...
const VizagProperties = () => {
  ...
  const fetchProperties = async () => {
    try {
      ...
      const params = new URLSearchParams({
        page: currentPage,
        limit: 9,  // Changed from 12 to 9 properties per page
      ...
      const response = await fetch(`${API_URL}/properties?city=Visakhapatnam&${params}`);
  ...
  const fetchPropertyTypes = async () => {
    try {
      const response = await fetch(`${API_URL}/cities/Vizag/types`);
      if (response.ok) {
        const data = await response.json();
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.header}>
        <h1 style={styles.title}>Properties in Visakhapatnam (Vizag)</h1>
                ...
                <div style={styles.card} className="property-card">
                  <div style={styles.imageContainer}>
                    ...
                    <span style={styles.locationBadge}>
                      <FaMapMarkerAlt /> {property.location?.split(',')[0] || 'Vizag'}
                    </span>
...
export default VizagProperties;