...
const GoaProperties = () => {
  ...
  const fetchProperties = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/properties/city/Goa?${params}`);
      
      if (!response.ok) throw new Error('Failed to fetch properties');
      
      const data = await response.json();
      console.log('Goa properties fetched:', data.properties.length, 'Total:', data.pagination?.totalProperties);
  ...
  const fetchPropertyTypes = async () => {
    try {
      const response = await fetch(`${API_URL}/cities/Goa/types`);
      if (response.ok) {
        const data = await response.json();
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.header}>
        <h1 style={styles.title}>Properties in Goa</h1>
                ...
                <div style={styles.card} className="property-card">
                  <div style={styles.imageContainer}>
                    ...
                    <span style={styles.locationBadge}>
                      <FaMapMarkerAlt /> {property.location?.split(',')[0] || 'Goa'}
                    </span>
