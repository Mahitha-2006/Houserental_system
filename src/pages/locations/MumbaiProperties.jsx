...
const response = await fetch(`${API_URL}/properties?city=Bengaluru&${params}`);
  ...
  const fetchPropertyTypes = async () => {
    try {
      const response = await fetch(`${API_URL}/cities/Mumbai/types`);
      if (response.ok) {
        const data = await response.json();
  ...
  return (
    <div style={styles.page}>
      <div style={styles.header}>
        <h1 style={styles.title}>Properties in Mumbai</h1>
                ...
                <div style={styles.card} className="property-card">
                  <div style={styles.imageContainer}>
                    ...
                    <span style={styles.locationBadge}>
                      <FaMapMarkerAlt /> {property.location?.split(',')[0] || 'Mumbai'}
                    </span>
...
export default MumbaiProperties;