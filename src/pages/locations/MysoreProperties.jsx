...
const MysoreProperties = () => {
  ...
  const fetchProperties = async () => {
    try {
      ...
      const params = new URLSearchParams({
        page: currentPage,
        limit: 9,  // Changed from 20 to 9 properties per page
      ...
      console.log('Properties received:', data.properties?.map(p => ({
        id: p._id,
        title: p.title,
        city: p.city,
        type: p.type
  ...
  const fetchPropertyTypes = async () => {
    try {
      const response = await fetch(`${API_URL}/cities/Mysore/types`);
      if (response.ok) {
        const data = await response.json();
  ...
  // Filter properties by search term with better debugging
  ...
  const propertiesByCity = filteredBySearch.reduce((acc, prop) => {
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.header}>
        <h1 style={styles.title}>Properties in Mysore Region</h1>
        <p style={styles.count}>
          {pagination?.totalProperties || 0} properties available across Mysore, Coorg, Siddapura, and surrounding areas
        </p>
        ...
        <div style={styles.regionStats}>
          <span>📍 Cities: Mysore, Siddapura, Coorg, Kutta, Kannur, Madikeri, Kushalnagar</span>
        </div>
      </div>
      ...
      <div style={styles.searchSection}>
        <div style={styles.searchContainer}>
          ...
          <input
            ...
            placeholder="Search by property name, location, city, or description..."
            value={searchTerm}
      ...
      {/* Properties Grid - Grouped by City */}
      {filteredBySearch.length === 0 ? (
        ...
        <>
          <div style={styles.cityGroups}>
            {Object.entries(propertiesByCity).map(([city, cityProperties]) => (
              <div key={city} style={styles.cityGroup}>
                <h3 style={styles.cityHeader}>
                  {city} 
                  <span style={styles.cityCount}>({cityProperties.length} properties)</span>
                </h3>
                        ...
                        <div style={styles.info}>
                          <div style={styles.titleRow}>
                            ...
                            {property.rating > 0 && (
                              <div style={styles.rating}>
                                ...
                                <span style={styles.reviews}>({property.reviewCount || 0})</span>
                              </div>
                            )}
                          </div>
                          <p style={styles.locationText}>
                            <FaMapMarkerAlt style={styles.smallIcon} /> {property.location || property.city}
                          </p>
                          <p style={styles.description}>{property.description?.substring(0, 100)}...</p>
                          <div style={styles.details}>
                            <span><FaBed /> {property.bedrooms || 0} {property.bedrooms === 1 ? 'bed' : 'beds'}</span>
                            <span><FaBath /> {property.bathrooms || 0} {property.bathrooms === 1 ? 'bath' : 'baths'}</span>
                          ...
                          <div style={styles.priceRow}>
                            ...
                            <span style={styles.totalPrice}>
                              {formatPrice((property.price || 0) + (property.cleaningFee || 0) + (property.serviceFee || 0))} total
                            </span>
                          </div>
                        </div>
                      </div>
